...
public class Bard extends PlayerClass {
    public Bard(possibleClasses name) {
        super(name);
    }

    @Override
    public HashMap<String, Integer> initSkills() throws IOException {
        if (this.getName() == possibleClasses.BARD) {
            String pathToSkillFile = "src/main/java/Datas/SkillDatas/BardSkills";
    ...
    public HashMap<String, Integer> initStats() throws IOException {
        if (this.getName() == possibleClasses.BARD) {
            String pathToSkillFile = "src/main/java/Datas/StatDatas/BardStats";
            HashMap<String, Integer> skills = new HashMap<>();
            String line;
            BufferedReader skillFileReader = new BufferedReader(new FileReader(pathToSkillFile));
            while ((line = skillFileReader.readLine()) != null) {
